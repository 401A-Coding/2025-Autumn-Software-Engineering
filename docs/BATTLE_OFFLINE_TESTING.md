...
4. 如果 2 分钟内没有重新进入，系统自动判负
5. OnlineLobby 的"继续未完成对局"卡片会在 5 秒内自动消失
...
4. **期望结果**：
   ...
   - B 的对局状态变为结束，显示"对手长时间离线，本局判您获胜"
   - A 返回大厅，OnlineLobby 的"继续未完成对局"卡片消失（5 秒内）
...
**问题**：用户返回大厅后，看到"继续未完成对局"卡片，但对局已经自动结束（2 分钟后）。

**解决**：OnlineLobby 现在每 5 秒自动检查一次对局状态。如果对局已结束，卡片会自动消失。
