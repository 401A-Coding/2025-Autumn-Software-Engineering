...
class EngineStub {
    ...
    validateAndApply(board: any, turn: any, from: any, to: any) {
        return { ok: true, board, nextTurn: turn === 'red' ? 'black' : 'red', winner: null };
    }
}
...
describe('BattlesService', () => {
    ...
    it('cancelWaiting should fail after second player joins', () => {
        ...
        expect(snap.status).toBe('playing');
        expect(() => service.cancelWaiting(a, matchA.battleId)).toThrow(BadRequestException);
    });

    it('cancelWaiting should fail if non-creator tries to cancel waiting battle', () => {
        const creator = 501; const other = 502;
        const { battleId } = service.quickMatch(creator, 'pvp');
        // other user has not joined yet
        expect(() => service.cancelWaiting(other, battleId)).toThrow(BadRequestException);
    });
});
