...
export class AdminService {
    ...
    async banUser(id: number, dto: { reason?: string; days?: number }) {
        ...
        const bannedUntil = dto.days ? new Date(Date.now() + dto.days * 24 * 3600 * 1000) : null;
    ...
    async unbanUser(id: number) {
        const user = await this.prisma.user.findUnique({ where: { id } });
        if (!user) throw new NotFoundException('用户不存在');
        return this.prisma.user.update({ where: { id }, data: { isBanned: false, bannedUntil: null } });
    }
    ...
    async listPosts(opts: { q?: string; status?: string }) {
        ...
        return this.prisma.post.findMany({ where, take: 200, orderBy: { createdAt: 'desc' } });
    }
    ...
    async removePost(id: number, adminId?: number) {
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            const updatedPost = await tx.post.update({ where: { id }, data: { status: 'REMOVED' } });
    ...
    async restorePost(id: number) {
        ...
        if (!post) throw new NotFoundException('帖子不存在');
        return this.prisma.post.update({ where: { id }, data: { status: 'PUBLISHED' } });
    }

    async listLogs(opts: { adminId?: number }) {
        const where = opts.adminId ? { adminId: opts.adminId } : undefined;
        return this.prisma.adminActionLog.findMany({ where, take: 200, orderBy: { createdAt: 'desc' } });
    }

    async listReports(opts?: { targetType?: string; status?: string; page?: number; pageSize?: number }) {
        const where: any = {}
        if (opts?.targetType) where.targetType = String(opts.targetType).toUpperCase()
        if (opts?.status) where.status = opts.status
        ...
        if (postIds.length) {
            const posts = await this.prisma.post.findMany({ where: { id: { in: postIds } }, select: { id: true, title: true } })
        ...
        const mapped = reports.map(r => ({
            ...r,
            reporter: r.reporter ? { id: r.reporter.id, username: r.reporter.username } : undefined,
            postTitle: r.targetType === 'POST' ? (postsMap[r.targetId]?.title ?? null) : null,
        }))
